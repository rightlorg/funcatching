...
void MapEditor::ondockbuttonClicked()
{
	...
	selection = senderButton->whatsThis().toInt();

	// For staris
	if (selection >= 202 && selection <= 209) {

		statusImage->load(":/item/image/stair/stair-" + QString::number(selection - 200) + ".png");
	}
	else if (blockStatus == 1 && selection != 0){	//Haha selection == 0 will never happen!
		...
		statusLabel->setText(blockList.blocklist.key(selection));
	}
...
void MapEditor::initdockButtons()
{
	for (quint8 i = 1; i < blockList.blocklist.size(); ++i) {
		...
		newButton->setWhatsThis(QString::number(i));
	...
	//load stair
	for (int i = 2; i < 10; ++i) {			//the index of the stair images start from 2 to 10
		QPushButton *newButton = new QPushButton(ui->dockWidget);
		newButton->setMaximumSize(36, 36);
		newButton->setMinimumSize(36, 36);
		newButton->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Maximum);

		QString picPath = ":/item/image/stair/stair-" + QString::number(i) + ".png";

		newButton->setIcon(QIcon(picPath));
		newButton->setIconSize(QSize(32, 32));
		newButton->setWhatsThis(QString::number(i + 200));		//the ID of the special item starts from 200
		qDebug() << newButton->whatsThis();
		ui->verticalLayout->addWidget(newButton);
		dockbuttonList.append(newButton);
		connect(newButton, SIGNAL(clicked()), this, SLOT(ondockbuttonClicked()));
